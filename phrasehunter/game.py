...
class Game():
    ...
    def __init__(self,total_lives = 5, *args, **kwargs):
        ...
        self.list_of_phrases = [Phrase(item, self.player_guess, True, False) for item in self.phrase_list]
        self.selected_phrase = random.choice(self.list_of_phrases)
        # print(self.selected_phrase)
        Phrase(self.selected_phrase, self.player_guess, False, True )
        self.welcome_message()
        self.start_game()

    def __str__(self):
        return "{}".format(self.selected_phrase)
